...
{
    //（1）设置LCD屏幕显示第一区
    LCD_aotu(2,2,238,50,1);               //LCD指定区域凸起
    LCD_ShowString(92,20,RED,GRAY,p);       //红字
    //（2）设置LCD屏幕显示第二区
    LCD_aotu(2,50,238,318,0);             //LCD指定区域凹下
}
//=====================================================================
...
void LCD_showScreen_digital(uint16_t y,const int num)
{
    ...
    {
        NumToStr_int(num,a);
        LCD_ShowString(k,y,BLACK,GRAY,a);
        k=k+18;		//x坐标：8+10
    }
    if(num>9&&num<100)
    {
